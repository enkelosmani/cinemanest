...
import {getTVDetails, getTVImages} from "@/services/tv.js";
...
onMounted(() => {
  loading.value = true
  Promise.all([
    getTVDetails(route.params.id).then((res) => {
      tv.value = res
      loading.value = false
    }),
    getTVImages(route.params.id).then((res) => {
  ...
  <div>
      ...
      <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl">
        <div class="relative overflow-hidden rounded-lg lg:h-[600px]">
          <div class="absolute inset-0">
            <img :src="ImageUrl + tv?.backdrop_path" alt="" class="h-full w-full object-cover object-center">
          </div>
          ...
          <div
            ...
            <div class="flex flex-col justify-center h-full">
              <h2 class="text-xl font-bold text-white">{{ tv?.original_title }}</h2>
              <p class="mt-1 text-sm text-gray-300">{{ tv?.overview }}</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bg-white">
      <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        <h2 class="text-4xl w-full text-center mb-8">Gallery</h2>
